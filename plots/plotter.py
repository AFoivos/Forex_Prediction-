...
class ForexPlotter:
    ...
    def visualize_swing_points(
        ...
        for col in cols:
            plt.figure(figsize = figsize)
            plt.plot(
                plot_data[self.close_col],
                label = self.close_col,
                color = 'blue',
                linewidth = 1
            )
            ...
            plt.title(f"Swing Points - {col} (Last {periods} Bars)", fontsize = 20)
            plt.xlabel('DateTime', fontsize = 12)
            plt.ylabel(self.close_col, fontsize = 12)
            plt.legend()
            plt.grid(True, alpha = 0.3)
